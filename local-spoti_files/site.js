...
window.Choices = __webpack_require__(36);
...
$(function () {
  ...
  if ($('select.js-choices').length) {
    new Choices('select.js-choices', {
      searchEnabled: false
    });
  }

  /* enable user input for new playlist */
  $('.c-form').each(function () {
    var $form = $(this),
        $dropdown = $form.find('.choices');

    $dropdown.addClass('state-choose');

    /**
     *  on switch:
     *
     *  - reset text input
     *  - set state on form, which triggers re-render
     */

    $form.find('.js-toggle-playlist').on('click', function () {
      var _state = $form.attr('data-state'),
          _input = $form.find('input[type="text"]').first();

      if (_state === 'new') {
        $form.attr('data-state', 'choose');
        _input.val('');
      } else {
        $form.attr('data-state', 'new');
        _input.focus();
      }
    });

    /**
     *  on open custom dropdown:
     *
     *  - scroll to offset + 20
     */
    $form.find('.choices').on('click', function () {
      if (window.outerWidth > 375) return;

      var _offset = $(this).offset().top;
      $(window).scrollTop(_offset - 20);
    });
  });
});
